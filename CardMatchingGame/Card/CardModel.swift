...
class CardModel {
    ...
    private var cardNames: [CardName] = CardName.allCases
    private var numOfCards: Int = CardName.allCases.count
    ...
    public func getCards(shuffleCards: Bool) -> [Card] {
        ...
        for cardName in 0..<numOfCards {
            let newCard1: Card = Card(imageName: cardNames[cardName])
            let newCard2: Card = Card(imageName: cardNames[cardName])
    ...
    public func getCardsForDemo() -> [Card] {
        ...
        let firstNewCard1: Card = Card(imageName: cardNames[0])
        let firstNewCard2: Card = Card(imageName: cardNames[0])
        
        cardArr.append(firstNewCard1)
        cardArr.append(firstNewCard2)
        
        let secondNewCard1: Card = Card(imageName: cardNames[1])
        let secondNewCard2: Card = Card(imageName: cardNames[1])
